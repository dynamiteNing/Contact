...
const signUp = async function (email, name, hashed_password, quote, join_date){
    ...
    const result = await db('insert', 'member', { 'email': email, 'name': name, 'hashed_password': hashed_password, 'quote': quote, 'join_date': new Date(join_date), 'role': role, 'rooms': rooms });
    return result;
};
